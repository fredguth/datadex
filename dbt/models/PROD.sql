...
SELECT 
    ...
    MC.VL_CUSTO_DIRETO AS VL_CD,
    MC.VL_CUSTO_INDIRETO AS VL_CI,
    MC.VL_CUSTO_TOTAL AS VL_CT,
    MC.VL_CUSTO_TOTAL / TP.QT_PRODUCAO_CENTRO_CUSTO AS VL_UNIT
FROM {{ source('raw', 'TB_MAPAREL_PRODUCAO') }} MP
    INNER JOIN {{ source('raw', 'TB_MOVIMENTACAO_CUSTO') }} MC ON MP.CO_CENTRO_CUSTO = MC.CO_CENTRO_CUSTO
    AND MP.DT_PRODUCAO_CENTRO_CUSTO = MC.DT_MOVIMENTACAO_CUSTO
    LEFT JOIN {{ source('raw', 'TB_CENTRO_CUSTO') }} CC ON MC.CO_CENTRO_CUSTO = CC.CO_SEQ_CENTRO_CUSTO
    LEFT JOIN {{ source('raw', 'TB_UNIDADE_CENTRO_CUSTO') }} UCC ON CC.CO_UNIDADE_CENTRO_CUSTO = UCC.CO_SEQ_UNIDADE_CENTRO_CUSTO
    LEFT JOIN {{ source('raw', 'TB_PRODUCAO') }} TP ON MP.CO_CENTRO_CUSTO = TP.CO_CENTRO_CUSTO
    AND MP.DT_PRODUCAO_CENTRO_CUSTO = TP.DT_PRODUCAO_CENTRO_CUSTO
    LEFT JOIN {{ source('raw', 'TB_ITEM_PRODUCAO') }} IP ON TP.CO_ITEM_PRODUCAO = IP.CO_SEQ_ITEM_PRODUCAO
WHERE MC.VL_CUSTO_TOTAL > 0
    AND TP.QT_PRODUCAO_CENTRO_CUSTO > 0
    AND TP.ST_REGISTRO_ATIVO = 'S'
    AND CC.ST_REGISTRO_ATIVO = 'S'
    AND IP.NO_ITEM_PRODUCAO <> 'Não Informado'
    -- remover registros de teste
    AND UCC.CO_SEQ_UNIDADE_CENTRO_CUSTO NOT IN (3553, 4473) 
    AND MP.DT_PRODUCAO_CENTRO_CUSTO < '2024-02-01'
ORDER BY DT_PROD
