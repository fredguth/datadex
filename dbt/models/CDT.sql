SELECT
    ...
    SUM(DCC.VL_DESP_CENTRO_CUSTO) AS VL_DESP
FROM {{ source('raw', 'TB_DESPESA_CENTRO_CUSTO') }} DCC
INNER JOIN {{ source('raw', 'TB_DESPESA') }} DES ON DCC.CO_DESPESA = DES.CO_SEQ_DESPESA
INNER JOIN {{ source('raw', 'TB_CENTRO_CUSTO') }} CC ON DCC.CO_CENTRO_CUSTO = CC.CO_SEQ_CENTRO_CUSTO
INNER JOIN {{ source('raw', 'TB_UNIDADE_CENTRO_CUSTO') }} DUC ON DES.CO_UNIDADE_CENTRO_CUSTO = DUC.CO_SEQ_UNIDADE_CENTRO_CUSTO
INNER JOIN {{ source('raw', 'TB_ITEM_CUSTO') }} IC ON DES.CO_ITEM_CUSTO = IC.CO_SEQ_ITEM_CUSTO
INNER JOIN {{ source('raw', 'TB_GRUPO_CUSTO') }} GC ON IC.CO_GRUPO_CUSTO = GC.CO_SEQ_GRUPO_CUSTO
...
AND DES.DT_DESPESA BETWEEN '2013-01-01' AND '2024-01-31'
